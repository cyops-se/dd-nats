...
### dd-nats-logger
Subscribes to "system log" which are generated by using the framework methods ```ddsvc.logger.Log|Info|Trace|Error```. Calling these methods will print the message to the console as well as publish it to NATS subject: **```system.log```** with a message in the format: ```{"time": "[time in UTC]", "category": "[category]", "title": "[title]", "description": "[description]"}```. See [this section](./inner/dd-nats-logger/README.md) for more information on the ```dd-nats-logger``` user interface.
