...
It is possible to build all components in Windows 10 (possibly other versions too) with a single command after the following prerequisites have been settled:

- Go version >= 1.21
- Git for Windows
- NVM for Windows (node version manager to build UI)
- Cross compiler, i.e. mingw-w64 or similar (gcc must be available in PATH)

All directories are relative to the ```dd-nats``` root directory as cloned from github (typically in the same directory as this README file is located on your computer).

#### Clone repository
Open a Git shell and move to the directory where you want the repository located.

```
git clone https://github.com/cyops-se/dd-nats
```
...
npm i
npm run build
move dist ..\svcs\dd-ui\web\static
```

#### Build executables for Windows
A successfull build results in a new directory structure ```dd-nats\dist``` with subdirectories \[os]\[side], for example:

- dd-nats\dist\windows\inner\ (for windows executables intended for the inside of the diode)
- dd-nats\dist\linux\outer\ (for linux executables intended for the outside of the diode)
...
#### Build executables for Linux
To build Linux executables, make sure to have mingw-64 or similar cross-compiler installed and configured first. NOTE! Windows specific executables like ```dd-nats-opcda``` will not be built for Linux.
